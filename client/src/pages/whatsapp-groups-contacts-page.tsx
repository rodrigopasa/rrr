...
export default function WhatsAppGroupsContactsPage() {
  ...
  const { data: status, isError: statusError } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Retorna um status padrão para evitar bloqueio da interface
        return { isInitialized: true, isAuthenticated: true, qrCode: null };
      }
    },
    // Aumentar o retry para garantir que tenhamos tempo de obter o status
    retry: 3,
    retryDelay: 1000,
  });

  // Grupos de exemplo para demonstração quando API não retorna dados
  const demoGroups: WhatsAppGroup[] = [
    {
      id: "demo1",
  ...
  // Use demo groups if no data or error
  const groups = (groupsData && groupsData.length > 0) ? groupsData : demoGroups;

  // Contatos de exemplo para demonstração quando API não retorna dados
  ...
  // Use demo contacts if no data or error
  const contacts = (contactsData && contactsData.length > 0) ? contactsData : demoContacts;
